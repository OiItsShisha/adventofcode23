...
def workin_fnc(line, lst) -> None:
    v1, v2 = next((char for char in list(line) if char.isdigit())), next((char for char in reversed(list(line)) if char.isdigit()))
...
with open(Path("./d1data.txt"), encoding="utf8") as f:
    for line in f:
        ...
        for dig in str_digits:
            if dig in temp_line: temp_line = temp_line.replace(dig, f"{dig[0]}{str_digits[dig]}{dig[-1]}")
